"""CIFAR-10 + ResNet18 cross-dataset generalization experiment for FAD.

The CIFAR-10 training set is split deterministically into disjoint target
training (18k), target validation (2k), and decoder auxiliary (30k) subsets.
The official test set is reserved for target testing and the final attack.
CIFAR-100 supplies out-of-distribution images for the BiDO+ OE term.
"""
...
def output_paths(args):
    ...
    return {
        "root": root,
        "split": root / f"cifar10_split_seed{args.split_seed}.json",
        "target": root / "target",
...
def cifar10_transform(args, augment=False):
    if not augment:
        return build_transform("cifar10", args.img_size, args.in_channels)
...
def load_cifar10(args, train, augment=False):
    return datasets.CIFAR10(
        root=str(Path(args.data_root).expanduser()),
        train=train,
        transform=cifar10_transform(args, augment=augment),
...
def split_payload(args):
    ...
    expected_total = args.target_train_samples + args.target_val_samples + args.decoder_samples
    if expected_total != 50000:
        raise ValueError(
            "target_train_samples + target_val_samples + decoder_samples must equal 50000"
        )
    ...
    indices = rng.permutation(50000).tolist()
    ...
    payload = {
        ...
        "decoder_indices": indices[val_end:],
...
def make_subsets(args):
    full_train = load_cifar10(args, train=True)
    if len(full_train) != 50000:
        raise RuntimeError(f"Expected 50000 CIFAR-10 training images, found {len(full_train)}")
...
def load_checked_target(path, args):
    ...
    return load_target(path, args.num_classes, args.feature_dim, pretrained=False)
...
def train_target(args):
    ...
    payload = split_payload(args)
    augmented_train = load_cifar10(args, train=True, augment=args.target_augmentation)
    evaluation_train = load_cifar10(args, train=True, augment=False)
    ...
    official_test = limited_subset(
        load_cifar10(args, train=False), args.max_target_test
    )
    test_loader = make_loader(official_test, args, shuffle=False)
    oe_loader = None
    if args.enable_oe:
        oe_transform = cifar10_transform(args, augment=args.target_augmentation)
        oe_dataset = datasets.CIFAR100(
...
def attack(args, seed):
    ...
    seed_everything(seed)
    test_dataset = load_cifar10(args, train=False)
...
def summarize(args):
    ...
    for seed in args.seeds:
        ...
        if payload.get("dataset") != "cifar10" or payload.get("arch") != "resnet18_projected":
...
def parse_args():
    ...
    parser.add_argument("--output_root", default="cifar10_resnet18_generalization")
...
def validate_args(args):
    if args.target_train_samples + args.target_val_samples + args.decoder_samples != 50000:
        raise ValueError("The three CIFAR-10 training partitions must total 50000")
    if min(args.target_train_samples, args.target_val_samples, args.decoder_samples) <= 0:
        raise ValueError("All three CIFAR-10 training partitions must be non-empty")
    if args.in_channels != 3 or args.num_classes != 10:
        raise ValueError("CIFAR-10 requires --in_channels 3 and --num_classes 10")
